...
class WallBoxTransaction with _$WallBoxTransaction implements DatabaseModel {
  ...
  static Map<DateTime, Map<String, List<WallBoxTransaction>>> sortedForBills() {
    Map<DateTime, Map<String, List<WallBoxTransaction>>> result = {};

    final transactions = repo.getAll().toList(growable: false);

    for (int i = 0; i < transactions.length; i++) {
      final ta = transactions[i];
      final month = ta.startTimeStamp.monthOnly;
      final tagID = ta.tagID;
      if (!result.containsKey(month)) result[month] = {};
      if (!result[month]!.containsKey(tagID)) {
        result[month]![tagID] = List.empty(growable: true);
      }

      result[month]![tagID]!.add(ta);
    }
